...
if __name__ == '__main__':
    ...
    if architecture_name == 'ResNet50':
        blocks = [3, 4, 6, 3]
        model = arch.resnet(input_shape=(224, 224, 3), blocks=blocks)

    if architecture_name == 'ResNet101':
        blocks = [3, 4, 23, 3]
        model = arch.resnet(input_shape=(224, 224, 3), blocks=blocks)

    if architecture_name == 'ResNet152':
        blocks = [3, 8, 36, 3]
        model = arch.resnet(input_shape=(224, 224, 3), blocks=blocks)

    if debug == False:
        model.load_weights('F:/Artur/weights/ImageNet224x224/{}.h5'.format(architecture_name.lower()))
    ...
    while rl.count_res_blocks(model) != [2, 2, 2, 2]:
        ...
        # scores = [(27, 0.3147810532479489), (37, 0.13173839542634125), (59, 0.0963634005752032), (69, 0.12443806881186903), (79, 0.14477765886993865), (101, 0.06516991566161046), (111, 0.05867513449000139), (121, 0.05946264246355082), (131, 0.07923166435630624), (141, 0.10165206798533155), (163, 0.10509671025917633), (173, 0.13012239231915512)]
        # print(scores)
